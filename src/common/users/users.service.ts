...
import {
  validationDataUnique,
  validationDataNotFound,
...
export class UsersService {
  ...
  async create(
    ...
    try {
      if (validation.error) {
        ...
        return {
          users: await this.userService.save(createUserDto),
  ...
  async findOne(id: string): Promise<{ error; message?; user? }> {
    const validation = await validationDataNotFound(
      this.userService,
      { username: id },
      'Retviewing data',
    );
